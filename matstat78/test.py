...
def hyp_testing(distName, param=0):
    ...
    for i in range(len(size)):
        ...
        for index in range(k):
            if (index == 0):
                print(index+1,"& ($-\infty$, ", deltas_r[index+1], "] & ",
                      P_r[index], " & ",
                      np_k[index], " & ",
                      n_k[index], " & ",
                      round(n_k[index] - np_k[index], 2), " & ",
                      round(n_k[index] - np_k[index] ** 2, 2), " & ",
                      chi_Bs_r[index], "\\\\ \\hline")
            elif (index == k-1):
                print(index+1,"& [", deltas_r[index], ", $\infty$) & ",
                      P_r[index], " & ",
                      np_k[index], " & ",
                      n_k[index], " & ",
                      round(n_k[index] - np_k[index], 2), " & ",
                ...
                print(index+1,"& [", deltas_r[index], ", ", deltas_r[index+1], "] & ",
                      P_r[index], " & ",
                      np_k[index], " & ",
                      n_k[index], " & ",
                      round(n_k[index] - np_k[index], 2), " & ",
                      round(n_k[index] - np_k[index] ** 2, 2), " & ",
                      chi_Bs_r[index], "\\\\ \\hline")
        print("\multicolumn{2}{|i|}{\sum} & ", round(sumP,2), " & ", round(sum(np_k),2), " & ", sumN_K, " & ", round(summ1,2), " & ", round(summ2,2), " & ", round(chi_B,2), "\\\\ \\hline")
    return results
